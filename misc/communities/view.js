...
// accept ?community= and ?c=
// i used ?c= while making the site but it looked like ass
const communityname = params.get("community") || params.get("c") || "";
if (communityname && params.get("c") && !params.get("community")) {
    const u = new URL(location.href);
    u.searchParams.delete("c");
    u.searchParams.set("community", communityname);
    history.replaceState(null, "", u);
}
const validtabs = ["top", "tweets", "replies", "media", "users"];
const initialtab = validtabs.includes(params.get("tab")) ? params.get("tab") : "top";
...
if (!communityname) {
    ...
    statusel.innerHTML =
        '<div class="emptystate">' +
            ...
            '<div>no community selected.<br><a href="./">browse the list</a></div>' +
        '</div>';
} else {
    init();
}
...
async function init() {
    const folder = encodeURIComponent(communityname);
    try {
        statusel.textContent = "loading community...";
        const idxres = await fetch("communities.json");
        if (!idxres.ok) throw new Error("communities.json HTTP " + idxres.status);
        const idx = await idxres.json();
        const c = (idx.communities || []).find(x => x.folder === communityname);
        if (!c) throw new Error("community " + communityname + " not found. like georgenotfound. since you got this error message likely through messing around this is your punishment");
        state.community = c;
        renderheader(c);
        document.title = c.name + " - community archive";
...
function updatetabcounts() {
    ...
    for (const tab of ["top", "tweets", "replies", "media", "users"]) {
...
function tabfilter(tab) {
    ...
    if (tab === "replies") return state.tweets.filter(t => t.in_reply_to && t.in_reply_to.status_id);
    if (tab === "media")   return state.tweets.filter(t => t.media && t.media.length);
    if (tab === "top")     return state.tweets.slice();
    return state.tweets.filter(t => !(t.in_reply_to && t.in_reply_to.status_id));
}
...
function updatefiltered() {
    ...
    resultcountel.textContent = fmtnum(state.filtered.length) + " result" + (state.filtered.length === 1 ? "" : "s");
...
function renderuser(u) {
    ...
    if (u.url) {
        // user.url is often a t.co; stub it for the global resolver
...
// my proxy has another use now!
const corsproxy = "https://cors.coolsite.cv/?url=";
...
async function opentweetdetail(tweetid, opts) {
    ...
    // syndicated tweets aren't in the archive: open the real tweet instead
    if (focal.syndicated) {
...
function closetweetdetail(opts) {
    ...
    detailbodyel.innerHTML = "";
    document.title = state.community ? (state.community.name + " - community archive") : "community archive";
...
// hook back button and browsr history popstate
