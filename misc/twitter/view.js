...
// each view.html sets window.ARCHIVE to declare what collection it is. the
// defaults keep this file working as the community viewer if nothing is set.
const CFG = Object.assign({
    kind: "community",            // "community" | "profile"
    param: "community",           // url query param that holds the folder
    index: "communities.json",    // index json sitting next to this view.html
    indexkey: "communities",      // array key inside that json
    matchkey: "folder",           // field matched against the url param
    titlesuffix: "community archive",
}, window.ARCHIVE || {});

const params = new URLSearchParams(location.search);
// accept ?<param>= and the legacy ?c=
const collectionname = params.get(CFG.param) || params.get("c") || "";
...
// tabs are whatever the html declares (profiles drop the members tab)
...
// for profiles the archive also holds quoted/parent/retweeted tweets by OTHER
// users (so threads + quotes resolve offline). those stay in byid for lookups
// but must not show up as feed items or inflate the tab counts.
function ownsfeed(t) {
...
// profile-header variants, sized by .pmetaicon (the umetaicon ones above are
// scoped to .userrow and would render unconstrained here). official x icons.
